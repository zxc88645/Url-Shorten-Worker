...
const github_version =
  typeof GITHUB_VERSION != "undefined" ? GITHUB_VERSION : "@main";
// 密码，密码正确情况无视白名单和超时设置，且支持自定义短链接，
const password = typeof PASSWORD != "undefined" ? PASSWORD : "AoEiuV020 yes";
// 短链超时，单位毫秒，支持整数乘法，0表示不设置超时，
const shorten_timeout =
  typeof SHORTEN_TIMEOUT != "undefined"
    ? SHORTEN_TIMEOUT.split("*").reduce((a, b) => parseInt(a) * parseInt(b), 1)
    : 1000 * 60 * 10;
// 默认短链key的长度，遇到重复时会自动延长，
const default_len =
  typeof DEFAULT_LEN != "undefined" ? parseInt(DEFAULT_LEN) : 6;
...
const remove_completely =
  typeof REMOVE_COMPLETELY != "undefined" ? REMOVE_COMPLETELY === "true" : true;
// 白名单中的域名无视超时，json数组格式，写顶级域名就可以，自动通过顶级域名和所有二级域名，
const white_list = JSON.parse(
  typeof WHITE_LIST != "undefined"
    ? WHITE_LIST
    : `[
"aoeiuv020.com",
"aoeiuv020.cn",
...
async function save_url(url, key, admin, len) {
  ...
  if (override || !is_exists) {
    ...
    if (
      remove_completely &&
      mode != 0 &&
      !(await checkWhite(new URL(url).host))
    ) {
...
async function handleRequest(request) {
  ...
  if (request.method === "POST") {
    ...
    if (
      ...
      (!admin && !demo_mode && !(await checkWhite(new URL(req["url"]).host)))
    ) {
    ...
    let stat,
      random_key = await save_url(req["url"], req["key"], admin);
